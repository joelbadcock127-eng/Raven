...
const PAGE_SCHEMA = {
  ...
  properties: {
    ...
    intro: { type: 'string', description: '2-3 sentence opening paragraph' },
    whyStay: { type: 'array', items: { type: 'string' }, description: '3-5 short reasons to stay at this property for this event' },
    plan: { type: 'array', items: { type: 'string' }, description: '3-4 line suggested itinerary around the event' },
    cta: { type: 'string', description: 'Button label, e.g. Check availability' },
    metaDescription: { type: 'string' },
  },
  required: ['headline', 'subheadline', 'intro', 'whyStay', 'plan', 'cta', 'metaDescription'],
...
export async function generateCampaignKit(
  ...
  const { data: campaign } = await supabase
    .from('campaigns')
    .select('id, property_id, event_id, status, kit, events(id, title, description, start_date, end_date, venue_name, locality, organiser, ticket_url, url, tags, ai_summary, estimated_audience)')
    .eq('id', campaignId)
    .maybeSingle();
  ...
  // media for the page hero + reel
  const { data: media } = await supabase
    .from('media_assets')
    .select('id, kind, public_url, times_used')
  ...
  const pageRes = await client.messages.create({
    ...
    max_tokens: 2000,
    system:
      'You write original landing pages for boutique Tasmanian accommodation, targeting people attending a specific local event. ' +
      'Warm, concrete, no hype clichés. Never copy source text — write fresh from the facts given. Australian English.',
    messages: [
      {
        role: 'user',
        content: `Property: ${prop.name} — ${prop.pitch}\nEvent facts: ${JSON.stringify(facts)}\nWrite the landing page content.`,
      },
    ],
    output_config: { format: { type: 'json_schema', schema: PAGE_SCHEMA } },
  });
  const pageContent = JSON.parse(pageRes.content.find((b) => b.type === 'text')?.text ?? '{}');
  ...
  const bundleRes = await client.messages.create({
    ...
    system:
      ...
      'Write original copy from the facts given — never reuse source text. Australian English.',
    messages: [
      {
        role: 'user',
  ...
  await supabase.from('event_pages').upsert({
    ...
    content: {
      ...pageContent,
      heroImageUrl: heroImage?.public_url ?? null,
